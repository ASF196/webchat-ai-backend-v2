...
// Uploads a custom chat-icon image (a client's logo, say) to Cloudinary and
// returns its URL — separate from the PATCH /bots/:token below because this
// one accepts multipart/form-data (a file), not JSON. The frontend then
// sends the returned URL along in a normal PATCH as iconDataUrl.
router.post('/bots/icon-upload', requireAuth, requireRole('owner', 'worker'), iconUpload.single('file'), async (req, res) => {
  try {
...
router.post('/bots', requireAuth, requireRole('owner', 'worker'), async (req, res) => {
  try {
    ...
    const data = {
      ...
      // Defaults to a protective daily cap rather than unlimited — see the
      // comment on daily_token_limit in the PATCH route below for why.
      // Adjust or clear this per client from Customize once they're on a
      // paid Groq plan (or a contract that covers heavier usage).
      daily_token_limit: 45000, overage_action: 'block',
...
router.patch('/bots/:token', requireAuth, requireRole('owner', 'worker'), requireBotAccess('token'), async (req, res) => {
  try {
    const fieldMap = {
      ...
      // Usage cap ("this client's plan includes N tokens/day"). Agency-only
      // — never client-editable, since it's a contract/billing lever, not
      // an appearance setting. null/0 = unlimited (only sensible once
      // you're comfortably on a paid Groq plan, or for a client you're
      // deliberately not capping). overageAction 'block' stops answering
      // with a friendly message once the cap is hit for the day; 'allow'
      // lets requests through past the cap — meant for once you're tracking
      // overage for billing rather than trying to stay inside a free tier.
      dailyTokenLimit: 'daily_token_limit', overageAction: 'overage_action',
    };
    const updates = {};
