...
router.patch('/agent', async (req, res) => {
  try {
    ...
    const allowed = { name: 'name', greeting: 'greeting', colorGrad: 'color_grad', iconKey: 'icon_key', orbGlow: 'orb_glow', glowColor: 'glow_color', humanHandoffEnabled: 'human_handoff_enabled' };
    const updates = {};
    for (const [bodyKey, col] of Object.entries(allowed)) {
      if (req.body[bodyKey] !== undefined) {
        const isBool = col === 'orb_glow' || col === 'human_handoff_enabled';
        updates[col] = isBool ? !!req.body[bodyKey] : String(req.body[bodyKey]).slice(0, 300);
      }
    }
