...
win.querySelector('#sp-reset-btn').onclick=function(){
  ...
  stopHumanPolling();
  // Reset needs to forget this visitor's identity too, not just clear the
  // screen — visitorId is what /api/human-messages/:token/:visitorId uses
  // to replay an old human-handoff thread on page load ("returning visitor
  // check" above). Without rotating it, reloading right after a reset
  // would still fetch the SAME old thread and put it right back on screen,
  // making the reset look like it never happened.
  visitorId='v_'+Math.random().toString(36).slice(2)+Date.now().toString(36);
  try{localStorage.setItem('sp_visitor_'+c.token,visitorId);}catch(e){}
